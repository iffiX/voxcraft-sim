...
class VX3_Link {
  public:
    ...
    __device__ static Vfloat axialStiffness(VX3_Context &ctx, Vindex link);
     ...
     */
    template <typename T>
    __device__ static VX3_Vec3D<T> toAxisX(LinkAxis axis, const VX3_Vec3D<T> &v) {
        switch (axis) {
        case Y_AXIS:
     ...
     */
    template <typename T>
    __device__ static VX3_Quat3D<T> toAxisX(LinkAxis axis, const VX3_Quat3D<T> &q) {
        switch (axis) {
        case Y_AXIS:
     ...
     */
    template <typename T>
    __device__ static VX3_Vec3D<T> toAxisOriginal(LinkAxis axis, const VX3_Vec3D<T> &v) {
        switch (axis) {
        case Y_AXIS:
            return VX3_Vec3D<T>(-v.y, v.x, v.z);
        case Z_AXIS:
