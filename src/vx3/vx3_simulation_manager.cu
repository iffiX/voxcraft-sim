...
bool VX3_SimulationManager::runSim(const VX3_Config &config, int device_index,
                                   int max_steps) {
    VcudaSetDevice(device_index);
    VX3_VoxelyzeKernelManager kernel_manager;
    kernel = kernel_manager.createKernelFromConfig(config);
...
void VX3_SimulationManager::enlargeGPUHeapSize(Vfloat heap_ratio) {
    // GPU Heap is for in-kernel malloc(). Refer to
    // https://stackoverflow.com/a/34795830/7001199

    // Not used now since we don't do in-kernel malloc yet.
    ...
    std::cout << boost::format{"Total GPU memory %ld bytes.\n"} % total << std::endl;
    heap_size_in_bytes = size_t(heap_ratio * (Vfloat)total);
    std::cout << boost::format{"Set GPU heap size to be %ld bytes.\n"} %
                     heap_size_in_bytes
              << std::endl;
    VcudaDeviceSetLimit(cudaLimitMallocHeapSize, heap_size_in_bytes);
...
void VX3_SimulationManager::saveResult() {
    ...
    kernel.ctx.voxels.read(voxels);
    kernel.ctx.voxel_materials.read(voxel_materials);
...
void VX3_SimulationManager::saveRecord() {
    ...
    VcudaMemcpy(tmp_steps, kernel.d_steps, sizeof(unsigned long) * kernel.frame_num,
                cudaMemcpyDeviceToHost);
    VcudaMemcpy(tmp_time_points, kernel.d_time_points, sizeof(Vfloat) * kernel.frame_num,
                cudaMemcpyDeviceToHost);
    VcudaMemcpy(tmp_link_record, kernel.d_link_record,
                sizeof(VX3_SimulationLinkRecord) * kernel.ctx.links.size() *
                    kernel.frame_num,
                cudaMemcpyDeviceToHost);
    VcudaMemcpy(tmp_voxel_record, kernel.d_voxel_record,
